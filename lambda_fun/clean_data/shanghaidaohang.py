...
class ShangHaiDaoHangCleaner(Base):
    ...
    def goodsflow(self):
        ...
        result = (
            flow.merge(store, on="orgcode", how="left")
            .merge(goods, on="plucode", how="left")
            .merge(
                gclass,
                left_on="clscode_1",
                right_on="clscode",
        ...
        result["foreign_category_lv1"] = result.apply(
            lambda row: row["clscode_lv1"] if row["clscode_lv2"] else "", axis=1
        )
        result["foreign_category_lv2"] = result.apply(
            lambda row: row["clscode_lv2"] if row["clscode_lv2"] else "", axis=1
        )

        result["foreign_category_lv3"] = result.apply(
            lambda row: row["clscode_lv3"] if row["clscode_lv3"] else "", axis=1
        )
        result["foreign_category_lv4"] = result.apply(
            lambda row: row["clscode_lv4"] if row["clscode_lv4"] else "", axis=1
        )
        result["foreign_category_lv5"] = result.apply(
            lambda row: row["clscode_lv5"] if row["clscode_lv5"] else "", axis=1
        )
    ...
    def cost(self):
        ...
        result = (
            cost.merge(store, on="orgcode", how="left")
            .merge(goods, on="plucode", how="left")
            .merge(
                gclass,
                left_on="clscode_1",
        ...
        result["total_cost"] = result.apply(
            lambda row: row["cost"] + row["adjustprofit"], axis=1
        )
        result["source_id"] = self.source_id
        result["cmid"] = self.cmid
        result["cost_type"] = ""
    ...
    def store(self):
        ...
        result = store.merge(area, on="shopareacode", how="left")

        orgtype_dict = {"0": "自营店", "1": "加盟店", "2": "配送中心"}

        result["property"] = result.apply(
            lambda row: orgtype_dict[row["orgtype"]], axis=1
        )
        result["cmid"] = self.cmid
        result["source_id"] = self.source_id
        result["address_code"] = ""
    ...
    def goods(self):
        ...
        result = (
            ...
            .merge(vendor, on="vendorcode", how="left")
            .merge(brand, left_on="brand", right_on="brandcode", how="left")
        )
        ...
        result["warranty"] = result.apply(
            lambda row: row["warranty"].split(".")[0] if row["warranty"] else "0",
            axis=1,
        )
    ...
    def goods_loss(self):
        ...
        result = (
            header.merge(detail, on="differno", how="left")
            .merge(store, on="orgcode", how="left")
            .merge(item, on="plucode", how="left")
            .merge(
                gclass,
                left_on="clscode_1",
                right_on="clscode",
